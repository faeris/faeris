...
static int s_mesh_load_frame(struct SMD2Mesh* mesh,FILE* file)
{
	...
	for(i=0;i<header->m_iNumFrames;i++)
	{
		...
		for(j=0;j<header->m_iNumVertices;j++)
		{
			...
			frame->m_pVerts[j].m_iNormalIndice=(unsigned char)(*p_verts++);
		}
...
static int s_mesh_load_texcoord(struct SMD2Mesh* mesh,FILE* file)
{
	...
	for(i=0;i<texnu;i++)
	{
		mesh->m_pTexCoords[i].m_fU=texs[i].x/(float) tex_width;
		mesh->m_pTexCoords[i].m_fV=texs[i].y/(float) tex_height;
	}
	free(texs);
	return 0;
}
...
static void s_smd2frame_print(struct SMD2Frame* f,int nuverts,int indent)
{
	...
	for(i=0;i<nuverts;i++)
	{
		...
		printf("%f,%f,%f,%d\n",f->m_pVerts[i].m_fX,f->m_pVerts[i].m_fY,f->m_pVerts[i].m_fZ,f->m_pVerts[i].m_iNormalIndice);
	}
	s_print_indent(indent+1);
	printf("}\n");
	s_print_indent(indent);
...
static void s_smd2mesh_print_texcoord(struct SMD2Mesh* mesh,int indent)
{
	...
	{
		s_print_indent(indent+1);
		printf("%f %f\n",mesh->m_pTexCoords[i].m_fU,mesh->m_pTexCoords[i].m_fV);
	}
	s_print_indent(indent);
	printf("}\n");
}
